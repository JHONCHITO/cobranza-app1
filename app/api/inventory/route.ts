...
// POST /api/inventory
// - Crear item nuevo
// - Actualizar item cuando viene { id, _update: true, ...campos }
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Error del servidor al guardar inventario" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("Error en DELETE /api/inventory:", error);
    return NextResponse.json(
      { success: false, error: "Error del servidor al borrar item" },
      { status: 500 }
    );
  }
}
